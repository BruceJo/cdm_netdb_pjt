...
class TestInterface:
    ...
    def test_resourceinfo(self):
        ...
        if response['response']['code'] == 'resourceinfo' and response['response']['message'] == 'success':
            data = response['response']['data']['raw']
            request = create_request("resourceinfo", "set", {"raw": data})
...
def test_instanceinfo():
    request = create_request("instanceinfo", "get", {"instance": [{"uuid": "2483886"}]})  # uuid for test
    test_message(request)

    request = create_request("instanceinfo", "get", {"instance": []})
    test_message(request)


def test_volume():
    # request = create_request("volume", "get", {"instance": [{"uuid": "2483886"}]})  # uuid for test
    # test_message(request)
    #
    # request = create_request("volume", "get", {"instance": []})
    # test_message(request)
    request = create_request("volume", "create", {"instance": []})
    print(request)
    request = create_request("volume", "create_sanpshot_volume", {"instance": [{"uuid": "2483886"}]})
    print(request)
    request = create_request("volume", "delete", {"instance": [{"uuid": "2483886"}]})
    print(request)
    request = create_request("volume", "attach", {"instance": [{"volume_uuid": "2483886", "instance_uuid": "2483886"}]})
    print(request)
    request = create_request("volume", "detach", {"instance": [{"instance_uuid": "2483886"}]})
    print(request)


def test_snapshot():
    request = create_request("snapshot", "get", {"instance_volume": [
        {
            "instance": {"uuid": "2483886"},
            "volume": [{"uuid": "3456789", "snapshot": [{"uuid": "1234567"}]}]
        }
    ]})
    test_message(request)
